        ...
        with conn.cursor() as cursor:
            ...
            for row in rows:
                comment = {
                    ...
                    "reply_text": row[4],
                    "risk_score": row[5],
                    "schedule_time": row[6].isoformat() if row[6] and hasattr(row[6], 'isoformat') else row[6],
                    "recommended_time": row[7].isoformat() if row[7] and hasattr(row[7], 'isoformat') else row[7],
                    "post_status": row[8],
                    "user_id": row[9],
                    "id": row[10],
